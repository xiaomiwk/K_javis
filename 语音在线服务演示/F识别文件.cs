...
namespace 语音在线服务演示
{
    public partial class F识别文件 : UserControl
    {
        ...
        string GRM_FILE = "gm_continuous_digit.abnf"; //构建离线识别语法网络所用的语法文件
        ...
        void 输入数据()
        {
            byte[] __PCM数据 = File.ReadAllBytes(Path.Combine(Environment.CurrentDirectory, "wav\\iflytek01.wav"));//打给零距离.wav//ddhgdw.pcm
            int __已解码长度 = 0;
            int __未解码长度 = __PCM数据.Length;
            while (true)
            {
                int __本次解码长度 = 6400;//3200与3600, 以及160这三个数值完整的业务关系未清楚
                if (__未解码长度 <= __本次解码长度)
                {
                    __本次解码长度 = __未解码长度;
                }

                byte[] __本次解码 = new byte[8000];
